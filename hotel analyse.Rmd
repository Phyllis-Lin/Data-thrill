...
type <- booking %>% 
  ...
  group_by(lead_time, arr_date_month) %>%
  count( name = "Number") 
...
ggplot(type, aes(x = factor(arr_date_month), y = lead_time, size = Number)) +
       geom_point(shape =21, colour = "#F8BBD0", fill = "#95CDF7") +    
...
```{r coordplot}
p <- ggplot(type, aes(x= reserved_room_type , y = log(Number*2), fill = assigned_room_type)) +
