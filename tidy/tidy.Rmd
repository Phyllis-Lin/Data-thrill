...
book_day <- final %>%
  ...
  dplyr::select(reservation_status_date, adr) %>%
  arrange((adr)) %>%
  filter(adr != 0) %>%
  head(10) %>%
  as_tibble() %>%
...
to_map <- final %>%
    ...
    dplyr::select(Country, total_bookings, contains("perc")) %>%
    pivot_longer(
      cols = contains("perc"),
      names_to = "outcome",
...
```{r}
dt1 <-final %>%
  dplyr::select(hotel,customer_type) %>%
  count(customer_type)
...
age1<- final%>%
  dplyr::select(hotel,adults,children,babies) %>%
  filter(hotel=="Resort Hotel") %>%
...
```{r}
night <- final  %>%
  dplyr::select(hotel,stays_in_week_nights) 
...
night1 <- final %>%
  dplyr::select(hotel,stays_in_weekend_nights) 
...
```{r}
month <- final %>%
  dplyr::select(hotel,arrival_date_month,arrival_date_year) %>%
  filter(hotel=="Resort Hotel")
